...
public class VoucherHelper implements SQLConverter {
	...
	public List<?> convertRSToList(ResultSet rs) {
		...
		try {
			while (rs.next()) {
				...
				voucherList.add(new Voucher(vid, cid, winnerUid, code, date, status));
			}
	...
	public void setColumnsForUpdate(Object entity, PreparedStatement stmt) {
		...
		try {
			stmt.setInt(1, voucher.getVid());
			stmt.setInt(2, voucher.getCid());
			stmt.setInt(3, voucher.getWinner_uid());
			stmt.setString(4, voucher.getCode());
			stmt.setLong(5, voucher.getDate());
	...
	public List<Voucher> select() {
		return (List<Voucher>) QueryHelper.ExecuteSelect(
